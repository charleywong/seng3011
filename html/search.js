...
$(document).ready(function() {
  var ajaxCallback = function(result, status) {
    ...
    if (result.news.length === 0)
      result.news = [{ headline: "We couldn't find any news", newsText: "" }];
  ...
  $("#submitRequest").click(function(event) {
    ...
    if (parseInt(upperWindow) + parseInt(lowerWindow) >= largeQueryBound) {
      sweetAlert({
        title: "Query upper and lower windows exceeds " + largeQueryBound,
    ...
    $.get(API_ENDPOINT, formData, function(result, status){return ajaxCallback(result, status)}); //function(result, status) {
    //     $(".loader").hide();
    //     $(document).scrollTop($(".animate-bottom").position().top);
    //     if (result.hasOwnProperty("error")) {
    //         var errorThrown = "Error: " + result.error + "\n";
